...
public class CampaignRepositoryImpl implements CampaignRepository {
	...
	public Campaign getFullCampaign(String id) {
		List<Map<String, Object>> rows = jdbcTemplate.queryForList("select campaign.id as campaign_id, campaign.name as campaign_name, campaign.username as campaign_username, race.name as race_name, class.name as class_name, `character`.* from campaign join campaign_character on campaign_character.campaign_id = campaign.id join `character` on campaign_character.character_id = `character`.id join class on `character`.class_id = class.id join race on `character`.race_id = race.id where campaign.id = ?", id);
		if(rows.isEmpty()) {
			//Temp
			return new Campaign();
		}
